...
4. **Configure GitCloud in Nextcloud Settings.**
   ...
   - **Settings > Personal > GitCloud** lets each user download a backup of their GitCloud commit history (a `.tar.gz` of the underlying `.git` directory, not their files) for safekeeping outside Nextcloud, or permanently delete that history (a full Git history wipe, not just the visible list) to reclaim disk space; their current files are not affected either way.
5. **GitCloud's `.git` repository is protected, with one known gap, from being modified directly through Nextcloud itself.** It lives at the root of your Nextcloud "files" storage, so unlike your other files it is not something GitCloud can hide — it can appear as an ordinary folder in the Files app, WebDAV, and non-official sync clients (the official Nextcloud desktop client already excludes `.git` folders from sync by default). GitCloud blocks deleting or renaming/moving anything under that folder through those channels — verified in a real browser against the Files app — since doing so outside GitCloud's own git plumbing could corrupt its history. Blocking *writes* (editing a file's content) is only verified working for a directly-authenticated WebDAV client (a sync client, `curl`, etc.) — a write made through the Files web app's own browser-based upload (drag-and-drop, "New > Upload file") is **not currently blocked**, a gap discovered during live testing whose root cause wasn't identified (see CHANGELOG 0.2.6). Use the Files app for your own files as usual, and the Settings > Personal backup/delete actions above to manage GitCloud's history specifically.

## Current State

- **App Name:** GitCloud
- **Base Directory:** `gitcloud`
- **Status:** Phase 1 and Phase 2 complete, plus admin/personal Settings pages (max file size limit, personal commit-history backup/deletion) and automatic tracking of files deleted/renamed/moved outside GitCloud. All Phase 3 prerequisites are cleared and Phase 3 (advanced features) is ready to begin. This README will be updated as features are implemented.
